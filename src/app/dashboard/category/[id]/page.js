...
export default function Home({ params }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let url = `https://star-food-bay.vercel.app/api/dashboard/category?id=${kitchenId}`;
    ...
    const getName = async () => {
        try {
            ...
            let response = await axios.get(
                `https://star-food-bay.vercel.app/api/dashboard/kitchen?id=${kitchenId}`
            );
  ...
  const handleAddClick = () => {
    try {
      let url = `https://star-food-bay.vercel.app/api/dashboard/category`;
  ...
  const handleDeleteClick = async (id) => {
    try {
      axios.delete(`https://star-food-bay.vercel.app/api/dashboard/category/${id}`);
      setRefresh((prevRefresh) => prevRefresh + 1);
    } catch (error) {
  ...
  const handleEditClick = () => {
    try {
      let url = `https://star-food-bay.vercel.app/api/dashboard/category/${selectedProductId}`;
      axios
