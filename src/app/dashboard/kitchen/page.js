...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let url = `https://star-food-b8w8.vercel.app/api/kitchen?id=${search}`;
  ...
  const handleAddClick = () => {
    try {
      let url = `https://star-food-b8w8.vercel.app/api/kitchen`;
  ...
  const handleDeleteClick = async (id) => {
    try {
      axios.delete(`https://star-food-b8w8.vercel.app/api/dashboard/kitchen/${id}`);
      setRefresh((prevRefresh) => prevRefresh + 1);
    } catch (error) {
  ...
  const handleEditClick = () => {
    try {
      let url = `https://star-food-b8w8.vercel.app/api/dashboard/kitchen/${selectedProductId}`;
      axios
