...
export default function Home({ params }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let url = `https://star-food-bay.vercel.app/api/dashboard/product?id=${id}`;
    ...
    const getName = async () => {
      try {
        ...
        let response = await axios.get(
          `https://star-food-bay.vercel.app/api/dashboard/category?catId=${id}`
        );
  ...
  const handleAddClick = () => {
    try {
      let url = `https://star-food-bay.vercel.app/api/dashboard/product`;
  ...
  const handleDeleteClick = async (id) => {
    try {
      axios.delete(`https://star-food-bay.vercel.app/api/dashboard/product/${id}`);
      setRefresh((prevRefresh) => prevRefresh + 1);
    } catch (error) {
  ...
  const handleEditClick = () => {
    try {
      let url = `https://star-food-bay.vercel.app/api/dashboard/product/${selectedProductId}`;
      axios
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "Category",
      render: () => (
        <Link href={`/dashboard/category/${kitchen}`}>
